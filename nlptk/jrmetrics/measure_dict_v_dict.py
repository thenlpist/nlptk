...
class DictVsDict:
    ...
    def compute_dict_v_dict(self, dict1, dict2, threshold=0.8):
        result_dict = self.compare_dicts(dict1, dict2, threshold)
        aggregated_result = aggregate_similarity(result_dict)
        return aggregated_result



    def compare_dicts(self, dict1, dict2, threshold=0.8):
        """
        ...
        for key in all_keys:
            ...
            if isinstance(value1, dict) and isinstance(value2, dict):
                result[key] = self.compare_dicts(value1, value2, threshold)

            # Handle string comparison using Levenshtein distance
            elif isinstance(value1, str) and isinstance(value2, str):
                result[key] = self.compute_similarity_result(value1, value2, threshold)

            # Handle type mismatch
            elif type(value1) != type(value2):
                result[key] = self.type_mismatch(value1, value2)

            elif isinstance(value1, list) and isinstance(value2, list):
                if value1 == [] and value2 == []:
                    self.compute_similarity_result(value1, value2, threshold)
                elif value1 == [] or value2 == []:
                    result[key] = [self.assign_similarity_result(s1, s2) for s1, s2 in itertools.zip_longest(value1, value2, fillvalue="")]
                elif isinstance(value1[0], dict) and isinstance(value2[0], dict):
                    result[key] = [self.compare_dicts(v1, v2, threshold) for v1, v2 in zip(value1, value2)]
                elif isinstance(value1[0], str) and isinstance(value2[0], str):
                    result[key] = [self.compute_similarity_result(s1, s2, threshold) for s1, s2 in itertools.zip_longest(value1, value2, fillvalue="")]
    ...
    def type_mismatch(self, value1, value2):
        return {
    ...
    def compute_similarity_result(self, s1, s2, threshold):
    ...
    def assign_similarity_result(self, s1, s2):
        return {
            "value1": s1,
            "value2": s2,
            "similarity": 0.0,
...
if __name__ == "__main__":
    ...
    result_dict = dvd.compare_dicts(dict1, dict2)
