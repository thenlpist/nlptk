...
def aggregate_list_similarity(comparison_list, return_metrics=False):
    ...
    # Add aggregated metrics directly to the list items if they're dictionaries
    if all(isinstance(item, dict) for item in processed_list) and processed_list:
        for item in processed_list:
            if 'aggregated_similarity' not in item and 'similarity' not in item:
                item['aggregated_similarity'] = list_similarity
                item['aggregated_match'] = list_match
